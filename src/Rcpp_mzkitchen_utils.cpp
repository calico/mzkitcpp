...
String mzk_get_isotope_parameters(const List& params, bool debug=false) {
    ...
    if (params.containsElementNamed("atomSpecificMaxIsotopes")) {
      ...
      StringVector atomVector = atomSpecificMaxIsotopes["key"];
      IntegerVector maxIsotopesVector = atomSpecificMaxIsotopes["value"];

      for (unsigned int i = 0; i < atomVector.size(); i++) {
        String atomRString = atomVector[i];
        string atom = string(atomRString.get_cstring());
        int maxIsotopes = maxIsotopesVector[i];

        isotopeParameters->atomSpecificMaxIsotopes.insert(make_pair(atom, maxIsotopes));
      }
    }
