...
void record_set_to_msp_library(const String& mspLibraryPath,
  ...
  StringVector mzkitchen_notes = StringVector(metabolite_data.nrows(), "");
  ...
  if (metabolite_data.containsElementNamed("mzkitchen_notes")) {
    mzkitchen_notes = metabolite_data["mzkitchen_notes"];
  }
  ...
  for (unsigned int i = 0; i < metabolite_data.nrows(); i++) {
    ...
    String mzkitchen_notes_RString = mzkitchen_notes[i];
    string mzkitchen_notes_val= string(mzkitchen_notes_RString.get_cstring());
    ...
    if (fragment && fragment->nobs() > 0) {
      ...
      if (!mzkitchen_notes_val.empty()) {
        outputMspFileStream << "MzkitchenNotes: " << mzkitchen_notes_val << "\n";
      }
