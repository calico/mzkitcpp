...
//maven_core submodule dependencies
#ifndef libmzkit_shared_EXPORTS //defined in Makevars
#include "./maven_core/libmaven/secprocessor.h"
#else
...
shared_ptr<SECSearchParameters> getSECSearchParams(const List& sec_search_params, const bool& debug){
    ...
    if (sec_search_params.containsElementNamed("groupMinNumPeaks")) params->groupMinNumPeaks = sec_search_params["groupMinNumPeaks"];
...
vector<SECTrace*> getTraces(const DataFrame& data, shared_ptr<SECSearchParameters> params, const bool& debug){
  ...
  if (!data.containsElementNamed("id") || !data.containsElementNamed("fraction_num") || !data.containsElementNamed("abundance")) {
    Rcerr << "getTraces(data, debug): data does not contain columns named \"id\", \"fraction_num\", and \"abundance\". Returning empty vector of traces.";
    return traces;
  }
  ...
  StringVector traceSetIds = StringVector(ids.size());
  if (data.containsElementNamed("trace_set_id")) {
    traceSetIds = data["trace_set_id"];
  }
  ...
  string current_trace_set_id = "";
  string previous_trace_set_id = "";
  ...
  for (unsigned int i = 0; i < ids.size(); i++) {
    ...
    String current_trace_set_id_rstr = traceSetIds[i];
    current_trace_set_id = string(current_trace_set_id_rstr.get_cstring());
    ...
    if (previous_id != current_id && previous_id != "") {
      ...
      trace->traceSetId = previous_trace_set_id;
    ...
    previous_trace_set_id = current_trace_set_id;
  }
  ...
  trace->traceSetId = current_trace_set_id;
...
DataFrame SEC_peakgroups(const DataFrame& data,
  ...
  if (!data.containsElementNamed("trace_set_id")) {
    Rcerr << "SEC_peakgroups(data, parameters, debug): data does not contain columns named \"trace_set_id\". This column is necessary to be able to perform grouping correctly.";
    
    DataFrame empty_output = DataFrame::create(
      Named("id") = StringVector(0),
      Named("trace_set_id") = StringVector(0),
      Named("analyte_id") = StringVector(0),
      Named("biological_id") = StringVector(0),
      Named("fraction_num") = IntegerVector(0),
      Named("abundance") = NumericVector(0),
      Named("smoothed") = NumericVector(0),
      Named("peak_data") =  StringVector(0),
      Named("group_id") = IntegerVector(0),
      Named("area") = NumericVector(0),
      Named("smoothed_area") = NumericVector(0),
  ...
  map<string, vector<SECTrace*>> tracesByTraceSetId{};
  
  for (SECTrace* trace : traces) {
    if (tracesByTraceSetId.find(trace->traceSetId) == tracesByTraceSetId.end()) {
      tracesByTraceSetId.insert(make_pair(trace->traceSetId, vector<SECTrace*>{}));
    }
    tracesByTraceSetId[trace->traceSetId].push_back(trace);
  }
  
  if (debug) {
    Rcout << "tracesByTraceSetId:" << endl;
    for (auto it = tracesByTraceSetId.begin(); it != tracesByTraceSetId.end(); ++it) {
  ...
  for (auto it = tracesByTraceSetId.begin(); it != tracesByTraceSetId.end(); ++it) {
    ...
    for (SECTrace* trace : secGroups.secTraces) {
        ...
        traceSetId.insert(traceSetId.end(), tracePeakSummaryString.size(), trace->traceSetId);
  ...
  StringVector output_traceSetIds = StringVector(N);
  ...
  for (unsigned int i = 0; i < output_fractionNums.size(); i++){
    output_Ids[i] = id[i];
    output_traceSetIds[i] = traceSetId[i];
  ...
  DataFrame output = DataFrame::create(
    Named("id") = output_Ids,
    Named("trace_set_id") = output_traceSetIds,
