...
- **SEC_cosine_scores**: Computes pairwise cosine similarity scores between all SEC traces in an experiment.
- **SEC_diff_traces**: Computes differential SEC traces by comparing reference and comparison sample data.
- **SEC_get_trace**: Computes a size exclusion chromatography (SEC) trace with smoothed intensities and picked peaks.
- **SEC_peak_scores**: Computes pairwise similarity scores between all detected peaks across all SEC traces.
- **SEC_peakgroups**: Groups SEC peaks across multiple traces within trace sets for protein complex detection.
